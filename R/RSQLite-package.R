...
#' argument to \code{sqliteNewConnection} to control the mode of the databse
#' connection:
#' 
#' \describe{ \item{list("SQLITE_RWC")}{ open the database in read/write mode
#' and create the database file if it does not already exist}
#' 
#' \item{list("SQLITE_RW")}{open the database in read/write mode.  Raise an
#' error if the file does not already exist}
#' 
#' \item{list("SQLITE_RO")}{open the database in read only mode.  Raise an
#' error if the file does not already exist} }
#' @name sqliteSupport
NULL
