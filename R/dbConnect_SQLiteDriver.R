...
#' @param drv,conn An objected generated by [SQLite()], or an existing \code{\linkS4class{SQLiteConnection}}.
...
#'   The name of the database \emph{is} the file name,
#'   thus database names should be legal file names in the running platform.
...
#'   Possible values for `synchronous` are "off" (the default), "normal", or  "full".  Users have reported significant speed ups
#'   using `sychronous = "off"`, and the SQLite documentation itself implies considerable improved performance
#'   at the very modest risk of database corruption in the unlikely case of the operating system (\emph{not} the R application) crashing.
...
#' @param vfs Select the SQLite3 OS interface.
#'   See https://www.sqlite.org/vfs.html for details.
#'   Allowed values are `"unix-posix"`, `"unix-unix-afp"`, `"unix-unix-flock"`, `"unix-dotfile"`, and `"unix-none"`.
...
#' When parameter `extended_types = TRUE` date and time columns are directly mapped to corresponding R-types.
#' How exactly depends on whether the actual value is a number or a string:
#'
