...
#' @param value a data.frame (or coercible to data.frame) object or a file name (character).
#'   In the first case, the data.frame is written to a temporary file and then imported to SQLite;
...
#'   while nonmissing elements/cells retain their value.
#'   If the autoincrement column exists in the data frame passed to the `value` argument, the `NA` elements are overwritten.
