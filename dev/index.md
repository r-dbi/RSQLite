...
- Runtime capability:
  ...
  returns TRUE on supported builds; it performs a lazy registration
  attempt if not already registered.
- Statistics:
  [`sqliteHttpStats()`](https://rsqlite.r-dbi.org/dev/reference/sqliteHttpStats.md)
  returns a list: `bytes_fetched`, `range_requests`, `full_download`
  (TRUE if any connection fell back to full download).
